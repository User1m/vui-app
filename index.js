...
vui.request = function (json) {
  ...
  if (this.data.context) {
    var System = findKey(this.data.context, "system");
  ...
  var session = new vui.session(json.session);
...
vui.app = function (name) {
  if (!(this instanceof vui.app)) {
  ...
  this.request = function (request_json) {
    var request = new vui.request(request_json);
    var response = new vui.response(request.getSession());
    ...
    self.request(event)
      .then(function (response) {
        if (os.platform() === 'win32') {
          context.done(null, { res: response });
        } else {
      ...
      .catch(function (response) {
        if (os.platform() === 'win32') {
          context.done('Exception: ' + response);
        } else {
          callback(response);
        }
      });
  ...
  // for Azure Functions
  this.run = function (context, req) {
    return self.handler(req, context, null);
  };
